...
Find this presentation on our [GitHub](https://github.com/rladies/meetup-presentations_dublin)
...
2. Pushing to Your Repositories
...
# 2. Creating and Cloning a Repository
  ...
  - You can actually clone any repository you want, not only yours 
...
will help you remember what you were doing when you commited the 
changes
...
# Pro-tip #1 : You don't need to type GH username/password on every push

You can connect your local git to your GitHub account with an SSH key.
...
# 4. Connecting your local Git to GitHub via an SSH Key
...
# Pro tip #2 : You can just use a Git client
...
> [SourceTree](https://confluence.atlassian.com/get-started-with-sourcetree/install-sourcetree-847359094.html) gets a lot of praises among Data Engineers on my team.
...
```
  ...
  - Step 2. Clone it (from your github account, e.g. `github.com/your_user/forked_repository`), make the changes 
  and push it. 
  
  - Step 3. Make a `pull request` into the original repository
...
# 7. Using Branches
  ...
  - Step 1. First `git checkout master` and `git pull`.
  - Step 2. Create a new branch with `git checkout -b branch_name`. Now you're working on this branch (use `git branch` to see what branch you're on)
    
  - Step 3. Add your changes and commit as usual except the push step: 
  now you need to use `git push branch_name` to identify where 
...
# 8. Final Remarks 
  ...
  - What we learned today is enough for:
    - Creating and pushing to your own repositories
    - Creating in branches (which can be used by groups)
    - Contributing to other repositories
  
  but they have many more features that you'll learn in practice :)  
